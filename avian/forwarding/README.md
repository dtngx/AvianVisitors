...
Default install hosts the collage at `http://birdie.local/` on your LAN, no auth. The recipes below are independent. Pick what you need.
...
```yaml
rest:
  - resource: http://birdie.local/avian/api/birdnet-api.php?action=recent&hours=1
    scan_interval: 60
    sensor:
      - name: "Latest Bird"
